...
The Drive Service owns Clud file and folder metadata. It stores the directory
...
requests enter through Drive Service and are proxied to Storage.
...
| `DRIVE_STORAGE_HTTP_CONNECT_TIMEOUT` | `2s` | Storage connection timeout |
| `DRIVE_STORAGE_HTTP_READ_TIMEOUT` | `30s` | Storage upload/download read timeout |
...
`DRIVE_SERVICE_URL=http://host.docker.internal:8082` for the Gateway container.
...
Flyway owns the `drive_service` PostgreSQL schema. Active sibling names are
...
The service publishes JSON events to `DRIVE_EVENTS_TOPIC`, which defaults to
`drive.events.v1`. Events are sent after the database transaction commits.
